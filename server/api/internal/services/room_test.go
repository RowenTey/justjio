...
import (
	...
	httpPkg "github.com/RowenTey/JustJio/server/api/pkg/http"
...
func (s *RoomServiceTestSuite) SetupTest() {
	...
	s.mockHTTPClient = new(httpPkg.MockHTTPClient)
...
func (s *RoomServiceTestSuite) TestCreateRoomWithInvites_Success() {
	// Setup test data
	host := &models.User{ID: 1, Username: "host"}
	inviteesStr := []string{"2", "3"}
	room := &models.Room{Name: "Test Room", VenuePlaceId: "ChIJN1t_tDeuEmsRUsoyG83frY4"}
	...
	s.mockUserRepo.On("FindByIDs", mock.Anything, inviteesStr).Return([]models.User{
		{ID: 2}, {ID: 3},
	}, nil)
	...
	s.mockRoomRepo.On("Create", mock.Anything, room).Run(func(args mock.Arguments) {
		...
		r := args.Get(1).(*models.Room) // Get second argument (index 1) since first is context
		r.ID = "test-room-id"
	}).Return(nil)
...
func (s *RoomServiceTestSuite) TestCreateRoomWithInvites_HostNotFound() {
	// Setup test data
	room := &models.Room{Name: "Test Room", VenuePlaceId: "randomPlaceId"}
	...
	// Simulate an error
	s.mockUserRepo.On("FindByID", mock.Anything, "1").Return((*models.User)(nil), errors.New("user not found"))
...
func (s *RoomServiceTestSuite) TestGetRooms_Success() {
	...
	// Execute
	rooms, err := s.roomService.GetRoomsByUserId(context.Background(), userId, page)
...
func (s *RoomServiceTestSuite) TestUpdateRoom_VenueChanged_Success() {
	...
	s.mockRoomRepo.On("Update", mock.Anything, mock.AnythingOfType("*models.Room")).Return(nil).Run(func(args mock.Arguments) {
		arg := args.Get(1).(*models.Room) // Get second argument (index 1) since first is context
...
func (s *RoomServiceTestSuite) TestUpdateRoom_VenueNotChanged_Success() {
	...
	// Mock expectations
	s.mockRoomRepo.On("GetByID", mock.Anything, roomId).Return(room, nil)
	s.mockRoomRepo.On("Update", mock.Anything, mock.AnythingOfType("*models.Room")).Return(nil)
...
func (s *RoomServiceTestSuite) TestCloseRoom_Success() {
	...
	roomId := "1"
	userId := "1"
	room := &models.Room{ID: "1", HostID: 1, IsClosed: false, Consolidated: "CONSOLIDATED"}
...
func (s *RoomServiceTestSuite) TestCloseRoom_NotHost() {
	...
	roomId := "1"
	userId := "2"                                                          // Not the host
	room := &models.Room{ID: "1", HostID: 1, Consolidated: "CONSOLIDATED"} // Host ID is 1
...
func (s *RoomServiceTestSuite) TestCloseRoom_UnconsolidatedBills() {
	...
	roomId := "123"
	userId := "1"
	room := &models.Room{ID: roomId, HostID: 1, Consolidated: "UNCONSOLIDATED"}
...
func (s *RoomServiceTestSuite) TestRespondToRoomInvite_Rejected() {
	...
	// Mock expectations
	user := &models.User{ID: 2, NoOfPendingRoomInvites: 1}
...
func (s *RoomServiceTestSuite) TestJoinRoom_Success() {
	...
	roomId := "1"
	userId := "2"
	room := &models.Room{ID: "1", NoOfAttendees: 1, Users: []models.User{}}
...
func (s *RoomServiceTestSuite) TestJoinRoom_AlreadyInRoom() {
	...
	roomId := "1"
	userId := "2"
	// User with ID 2 is already in the room
	existingUser := models.User{ID: 2, Username: "user2"}
	room := &models.Room{ID: "1", NoOfAttendees: 1, Users: []models.User{existingUser}}
	...
	// Expect transaction rollback (since user is already in room)
	s.sqlMock.ExpectRollback()
...
func (s *RoomServiceTestSuite) TestLeaveRoom_Success() {
	...
	userId := "2"                                                          // Not the host
	room := &models.Room{ID: "1", HostID: 1, Consolidated: "CONSOLIDATED"} // Host ID is 1
	user := &models.User{ID: 2, NoOfRooms: 1}
...
func (s *RoomServiceTestSuite) TestLeaveRoom_AsHost() {
	...
	room := &models.Room{ID: "1", HostID: 1, Consolidated: "CONSOLIDATED"}
...
func (s *RoomServiceTestSuite) TestInviteUsersToRoom_Success() {
	...
	room := &models.Room{ID: "1", HostID: 1}
	inviter := &models.User{ID: 1}
	invitees := []models.User{{ID: 2}, {ID: 3}}
...
func (s *RoomServiceTestSuite) TestInviteUsersToRoom_NotHost() {
	...
	inviterId := "2"
	invitees := []string{"3", "4"}
	room := &models.Room{ID: "123", HostID: 1} // Host ID is 1, inviter ID is 2
...
func (s *RoomServiceTestSuite) TestCreateRoomWithInvites_EmptyRoomName() {
	...
	host := &models.User{ID: 1, Username: "host", NoOfRooms: 0}
	...
	s.mockRoomRepo.On("Create", mock.Anything, mock.AnythingOfType("*models.Room")).Run(func(args mock.Arguments) {
		r := args.Get(1).(*models.Room) // Get second argument (index 1) since first is context
		r.ID = "1"
	}).Return(nil)

	// Mock CreateInvites with room invites
	s.mockRoomRepo.On("CreateInvites", mock.Anything, mock.AnythingOfType("[]models.RoomInvite")).Return(nil)
	s.mockUserRepo.On("Update", mock.Anything, host).Return(nil)
...
func (s *RoomServiceTestSuite) TestInviteUsersToRoom_UserNotFound() {
	...
	room := &models.Room{ID: roomId, HostID: 1}
	s.mockRoomRepo.On("GetByID", mock.Anything, roomId).Return(room, nil)

	inviter := &models.User{ID: 1, Username: "host"}
	s.mockUserRepo.On("FindByID", mock.Anything, inviterId).Return(inviter, nil)

	// Mock GetRoomAttendeeIDs to return existing attendees (as []string)
	s.mockRoomRepo.On("GetRoomAttendeeIDs", mock.Anything, roomId).Return([]string{"1"}, nil)

	// Mock GetPendingInviteUsers to return no pending invites
	s.mockRoomRepo.On("GetPendingInviteUsers", mock.Anything, roomId).Return([]string{}, nil)

	// User not found
	s.mockUserRepo.On("FindByIDs", mock.Anything, inviteUserIds).Return([]models.User{}, gorm.ErrRecordNotFound)
...
func (s *RoomServiceTestSuite) TestJoinRoom_RoomNotFound() {
	...
	// Mock expectations
	s.mockRoomRepo.On("GetByIDWithAttendees", mock.Anything, roomId).Return((*models.Room)(nil), gorm.ErrRecordNotFound)
...
func (s *RoomServiceTestSuite) TestJoinRoom_PrivateRoomWithoutInvite() {
	...
	// Private room without user in it
	room := &models.Room{
		ID:        roomId,
		Name:      "Private Room",
		HostID:    1,
		IsPrivate: true,
		Users:     []models.User{}, // User not in room yet
	}
	s.mockRoomRepo.On("GetByIDWithAttendees", mock.Anything, roomId).Return(room, nil)

	// User to join
	user := &models.User{ID: 2, Username: "user2", NoOfRooms: 0}
	s.mockUserRepo.On("FindByID", mock.Anything, userId).Return(user, nil)
	s.mockUserRepo.On("Update", mock.Anything, user).Return(nil)
	s.mockRoomRepo.On("Update", mock.Anything, room).Return(nil)
...
func (s *RoomServiceTestSuite) TestLeaveRoom_UserNotInRoom() {
	...
	room := &models.Room{ID: roomId, HostID: 1, Consolidated: "CONSOLIDATED"}
	s.mockRoomRepo.On("GetByID", mock.Anything, roomId).Return(room, nil)
