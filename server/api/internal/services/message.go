...
func NewMessageService(
	...
	return &MessageService{
		...
		logger:      logger.WithFields(logrus.Fields{"service": "MessageService"}),
	}
}
...
func (ms *MessageService) SaveMessage(
	...
	return database.RunInTransaction(ms.db, sql.LevelDefault, func(tx *gorm.DB) error {
		...
		// TODO: Outbox pattern?
		broadcastPayload := kafka.KafkaMessage{
			MsgType: "CREATE_MESSAGE",
			Data: &kafka.MessagePayload{
				RoomID:     roomId,
				SenderID:   senderId,
...
func (ms *MessageService) GetMessagesByRoomId(
	ctx context.Context,
	roomId string,
	page int,
	asc bool,
) ([]response.MessageDto, int, error) {
