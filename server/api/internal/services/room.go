...
import (
	...
	"github.com/RowenTey/JustJio/server/api/pkg/dto/location"
...
func NewRoomService(
	...
	return &RoomService{
		...
		logger:           logger.WithFields(logrus.Fields{"service": "RoomService"}),
	}
}
...
func (rs *RoomService) CreateRoomWithInvites(
	...
	if err := database.RunInTransaction(rs.db, sql.LevelRepeatableRead, func(tx *gorm.DB) error {
		...
		invites := make([]models.RoomInvite, len(invitees))
		for i, user := range invitees {
			invites[i] = models.RoomInvite{
				RoomID:    room.ID,
				UserID:    user.ID,
				InviterID: host.ID,
				Status:    "pending",
...
func (rs *RoomService) GetUnjoinedPublicRooms(ctx context.Context, userId string) ([]response.SimplifiedRoomDto, error) {
	rooms, err := rs.roomRepo.GetUnjoinedRoomsByIsPrivate(ctx, userId, false)
	if err != nil {
		return nil, err
	}

	roomDtos := make([]response.SimplifiedRoomDto, len(rooms))
	for i, room := range rooms {
		roomDtos[i] = response.SimplifiedRoomDto{
			ID:            room.ID,
			Name:          room.Name,
			Time:          room.Time,
			Venue:         room.Venue,
...
func (rs *RoomService) GetRoomInvites(ctx context.Context, userId string) ([]response.RoomInviteDto, error) {
	...
	for i, invite := range invites {
		roomInviteDtos[i] = response.RoomInviteDto{
			...
			User: response.MinimalUserDto{
				ID:         invite.User.ID,
				Username:   invite.User.Username,
				PictureUrl: invite.User.PictureUrl,
			},
			Inviter: response.MinimalUserDto{
				ID:         invite.Inviter.ID,
				Username:   invite.Inviter.Username,
				PictureUrl: invite.Inviter.PictureUrl,
			},
			Room: response.SimplifiedRoomDto{
				...
				Host: response.MinimalUserDto{
					ID:         invite.Room.Host.ID,
					Username:   invite.Room.Host.Username,
					PictureUrl: invite.Room.Host.PictureUrl,
				},
			},
		}
	}
...
func (rs *RoomService) JoinRoom(ctx context.Context, roomId, userId string) (*response.RoomDto, error) {
	...
	if err := database.RunInTransaction(rs.db, sql.LevelRepeatableRead, func(tx *gorm.DB) error {
		...
		for _, u := range room.Users {
			if utils.UIntToString(u.ID) == userId {
...
func (rs *RoomService) InviteUsersToRoom(
	...
	return database.RunInTransaction(rs.db, sql.LevelRepeatableRead, func(tx *gorm.DB) error {
		...
		roomInvites := make([]models.RoomInvite, len(invitees))
		for i, invitee := range invitees {
			roomInvites[i] = models.RoomInvite{
				RoomID:    room.ID,
				UserID:    invitee.ID,
...
func (rs *RoomService) GetUninvitedFriendsForRoom(
	ctx context.Context,
	roomId string,
	userId string,
) ([]response.MinimalUserDto, error) {
...
func (rs *RoomService) QueryVenue(query string) ([]location.Venue, error) {
	if query == "" {
	...
	var predictions []location.Venue
