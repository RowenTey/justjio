...
func (s *UserServiceTestSuite) TestAcceptFriendRequest_AlreadyProcessed() {
	// Setup test data
	requestID := uint(1)
	processedRequest := &models.FriendRequest{
		ID:          requestID,
		Status:      "accepted",
...
func (s *UserServiceTestSuite) TestSendFriendRequest_Success() {
	...
	s.mockUserRepo.On("CreateFriendRequest", mock.Anything, mock.AnythingOfType("*models.FriendRequest")).Return(nil)
	s.mockUserRepo.On("UpdateNoOfPendingFriendRequests", mock.Anything, []uint{receiverID}, 1).Return(nil)
...
func (s *UserServiceTestSuite) TestSearchUsers_Success() {
	currentUserID := "1"
	query := "test"
	expected := []models.User{{ID: 2, Username: "testuser"}}
...
func (s *UserServiceTestSuite) TestGetUserByID_NotFound() {
	...
	// Mock expectations
	s.mockUserRepo.On("FindByID", mock.Anything, userID).Return((*models.User)(nil), gorm.ErrRecordNotFound)
...
func (s *UserServiceTestSuite) TestUpdateUserField_Success() {
	...
	existingUser := &models.User{ID: 1, IsOnline: false}
...
func (s *UserServiceTestSuite) TestCreateOrUpdateUser_Create() {
	// Setup test data
	newUser := &models.User{Username: "newuser"}
...
func (s *UserServiceTestSuite) TestMarkOnline_Success() {
	...
	// Mock expectations
	s.mockUserRepo.On("FindByID", mock.Anything, userID).Return(&models.User{ID: 1}, nil)
	s.mockUserRepo.On("Update", mock.Anything, mock.AnythingOfType("*models.User")).Return(nil)
...
func (s *UserServiceTestSuite) TestMarkOffline_Success() {
	...
	// Mock repository calls
	existingUser := &models.User{ID: 1, IsOnline: true}
	s.mockUserRepo.On("FindByID", mock.Anything, userID).Return(existingUser, nil)
	s.mockUserRepo.On("Update", mock.Anything, mock.AnythingOfType("*models.User")).Return(nil)
