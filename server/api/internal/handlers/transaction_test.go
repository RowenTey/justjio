...
type TransactionHandlerTestSuite struct {
	...
	testNotificationChan chan notifications.NotificationData
...
func (suite *TransactionHandlerTestSuite) SetupSuite() {
	...
	suite.testNotificationChan = make(chan notifications.NotificationData, 100)
	transactionRepo := repositories.NewTransactionRepository(suite.db)
	billRepo := repositories.NewBillRepository(suite.db)
	...
	subscriptionRepo := repositories.NewSubscriptionRepository(suite.db)
...
func (suite *TransactionHandlerTestSuite) SetupTest() {
	...
	bill := models.Bill{
		...
		Payers:          []models.User{payer, payee},
	}
