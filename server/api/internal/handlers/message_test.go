...
func (suite *MessageHandlerTestSuite) SetupSuite() {
	...
	suite.kafkaClient, err = kafka.NewKafkaClient(config, suite.logger, "test")
	assert.NoError(suite.T(), err)

	// Initialize deps
	suite.mockJWTSecret = "test-secret"
	mockHttpClient := new(httpPkg.MockHTTPClient)
	...
	messageRepo := repositories.NewMessageRepository(suite.db)
	roomRepo := repositories.NewRoomRepository(suite.db)
	...
	roomMiddleware := func(c *fiber.Ctx) error {
		return middlewares.IsUserInRoom(c, roomService)
	}
...
func (suite *MessageHandlerTestSuite) TestGetMessages_Success_Ascending() {
	// Create messages
	now := time.Now()
	msg1 := models.Message{RoomID: suite.testRoomID, SenderID: suite.testUser1ID, Content: "Msg 1", SentAt: now.Add(-time.Minute)}
	msg2 := models.Message{RoomID: suite.testRoomID, SenderID: suite.testUser2ID, Content: "Msg 2", SentAt: now}
