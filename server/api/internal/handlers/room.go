...
func (h *RoomHandler) CreateRoom(c *fiber.Ctx) error {
	ctx := otel.GetOtelContext(c)
	req := middlewares.GetValidatedRequest[request.CreateRoomRequest](c)
...
func (h *RoomHandler) EditRoom(c *fiber.Ctx) error {
	ctx := otel.GetOtelContext(c)
	req := middlewares.GetValidatedRequest[request.EditRoomRequest](c)
...
func (h *RoomHandler) RespondToRoomInvite(c *fiber.Ctx) error {
	ctx := otel.GetOtelContext(c)
	req := middlewares.GetValidatedRequest[request.RespondToRoomInviteRequest](c)
...
func (h *RoomHandler) InviteUser(c *fiber.Ctx) error {
	ctx := otel.GetOtelContext(c)
	req := middlewares.GetValidatedRequest[request.InviteUserRequest](c)
...
// @Success 200 {object} object{status=string,message=string,data=[]location.Venue} "Queried venues successfully"
