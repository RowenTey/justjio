...
func (suite *BillHandlerTestSuite) SetupSuite() {
	...
	billRepository := repositories.NewBillRepository(suite.db)
	roomRepository := repositories.NewRoomRepository(suite.db)
	userRepository := repositories.NewUserRepository(suite.db)
	transactionRepo := repositories.NewTransactionRepository(suite.db)
...
func (suite *BillHandlerTestSuite) SetupTest() {
	...
	room := models.Room{
		...
		Users:  []models.User{user1, user2}, // Add both users to the room
	}
	result = suite.db.Create(&room)
	assert.NoError(suite.T(), result.Error)
	suite.testRoomID = room.ID

	suite.logger.Infof("SetupTest complete: User1 ID=%d, User2 ID=%d, Room ID=%s", suite.testUser1ID, suite.testUser2ID, suite.testRoomID)
}
...
func (suite *BillHandlerTestSuite) TestCreateBill_RoomAlreadyConsolidated() {
	...
	bill := models.Bill{
		...
		Payers: []models.User{{ID: suite.testUser1ID}, {ID: suite.testUser2ID}},
	}
	err := suite.db.Create(&bill).Error
	assert.NoError(suite.T(), err)

	// 2. Mark the room as consolidated
	err = suite.db.Model(&models.Room{}).
...
func (suite *BillHandlerTestSuite) TestConsolidateBills_Success() {
	...
	bill1 := models.Bill{
		RoomID: suite.testRoomID, OwnerID: suite.testUser1ID, Name: "Bill 1", Amount: 100.00,
		IncludeOwner: true,
		Payers:       []models.User{{ID: suite.testUser2ID}}, // Both pay
	}
	bill2 := models.Bill{
		RoomID: suite.testRoomID, OwnerID: suite.testUser2ID, Name: "Bill 2", Amount: 50.00,
		IncludeOwner: true,
		Payers:       []models.User{{ID: suite.testUser1ID}}, // Only User1 pays
	}
	...
	suite.mockTransactionService.
		On("GenerateTransactions", mock.Anything, mock.AnythingOfType("*models.Consolidation")).
...
func (suite *BillHandlerTestSuite) TestConsolidateBills_AlreadyConsolidated() {
	bill1 := models.Bill{
		...
		Amount:       20.00,
		IncludeOwner: true,
		Payers:       []models.User{{ID: suite.testUser2ID}},
	}
	err := suite.db.Create(&bill1).Error
