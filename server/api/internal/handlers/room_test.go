...
func (suite *RoomHandlerTestSuite) SetupSuite() {
	...
	suite.mockHttpClient = new(httpPkg.MockHTTPClient)
...
func (suite *RoomHandlerTestSuite) SetupTest() {
	...
	err = suite.db.Model(&models.User{}).Where("id = ?", suite.testHostID).Update("no_of_rooms", 1).Error
	assert.NoError(suite.T(), err)
	...
	err = suite.db.Model(&models.User{}).Where("id = ?", suite.testUserID).Update("no_of_pending_room_invites", 1).Error
	assert.NoError(suite.T(), err)
...
func (suite *RoomHandlerTestSuite) TestGetRooms_Success() {
	...
	room2 := models.Room{
		...
		Name:   "Second Room",
		HostID: suite.testHostID,
		Users:  []models.User{{ID: suite.testHostID}},
	}
